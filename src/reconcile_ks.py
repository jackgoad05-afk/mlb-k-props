"""
Reconciliation for the K-props paper-trading ledger (output/ks_paper_ledger.csv).

Run the morning after games finish (or any time after):

    python src/reconcile_ks.py reconcile          # fill in actual_so/result/pnl/clv
    python src/reconcile_ks.py summary             # running totals
    python src/reconcile_ks.py summary --by-tier   # broken out by edge-size tier

`reconcile` fills in, per pending (result-blank) ledger row whose game date has passed:
  - actual strikeouts: MLB Stats API gameLog (free, always available once the game
    is final).
  - CLV: pure arithmetic on closing_over_odds/closing_under_odds, IF a row already has
    them. This build's free API tier has no /historical/ access, so closing prices
    aren't fetched here -- they come from capture_closing_ks.py, which snapshots them
    itself near first pitch and writes them into the ledger ahead of time. If a row's
    closing price was never captured (e.g. the capture script never ran, or the game
    started before the capture window), CLV stays blank for that row -- actual_so/
    result/pnl still fill in fine regardless, since those only need the free MLB API.
    Every reconcile run reports how many rows had a closing price to work with.
"""
...
def reconcile():
    ...
    ledger["result"] = ledger["result"].astype("object")  # CSV round-trips can coerce an
    # all-blank column to float64; force object dtype before writing strings into it.
    ...
    for idx, row in playable.iterrows():
        ...
        if actual_so is None or pd.isna(actual_so):
            n_no_result_yet += 1
            continue  # postponed/no-decision/not yet reflected upstream -- retry next run
    ...
    print(f"CLV: {n_has_closing} row(s) had a captured closing price, {n_no_closing} did not "
          f"(run capture_closing_ks.py before this to fill those in)")
...
def summarize(by_tier: bool = False):
    ...
    print(f"=== K props paper-trading ledger ===")
